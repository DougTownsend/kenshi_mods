[CmdletBinding()]
param(
    [string]$SharedRoot = 'Z:\show_effective_stats',
...
foreach ($variable in @('KENSHILIB_DIR', 'KENSHILIB_DEPS_DIR', 'BOOST_INCLUDE_PATH', 'BOOST_ROOT')) {
    if (-not [Environment]::GetEnvironmentVariable($variable, 'Process')) {
        ...
        if (-not $userValue) { throw "$variable is not set. Configure the KenshiLib dependency environment first." }
        [Environment]::SetEnvironmentVariable($variable, $userValue, 'Process')
    }
}

$env:SHOWEFFECTIVESTATS_WINDOWS_SDK_INCLUDE = $WindowsSdkInclude
$env:SHOWEFFECTIVESTATS_WINDOWS_SDK_LIB = $WindowsSdkLib
...
$project = Join-Path $StagingRoot 'project\ShowEffectiveStats.vcxproj'
...
$stagingOutput = Join-Path $StagingRoot 'build\Release\x64'
$sharedOutput = Join-Path $SharedRoot 'build\Release\x64'
$dll = Join-Path $stagingOutput 'ShowEffectiveStats.dll'
$pdb = Join-Path $stagingOutput 'ShowEffectiveStats.pdb'
foreach ($path in @($dll, $pdb)) {
    if (-not (Test-Path -LiteralPath $path)) { throw "Build output was not found: $path" }
}
